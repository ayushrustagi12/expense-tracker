    ...
    <Layout>
      <div className="p-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Accounts & Cards</h1>
            <p className="text-gray-600 mt-2">Manage your bank accounts and payment cards</p>
          </div>
        ...
        {/* Bank Accounts */}
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Bank Accounts</h2>
            ...
            {accounts.map((account) => (
              <div key={account.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-bold text-gray-900">₹{account.balance.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cards.map((card) => (
              <div key={card.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-4">
                ...
                {card.type === 'Credit Card' && (
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium">₹{card.used?.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-bold text-green-600">₹{((card.limit || 0) - (card.used || 0)).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">Billing Date</span>
                      <span className="text-sm font-medium">{card.billingDate}</span>
                    </div>
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between text-xs text-gray-500 mb-1">
                        <span>Usage</span>
                        <span>{Math.round(((card.used || 0) / (card.limit || 1)) * 100)}%</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className="bg-blue-500 h-2 rounded-full transition-all duration-300"
                          style={{ width: `${((card.used || 0) / (card.limit || 1)) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
                ...
                {card.type === 'Debit Card' && (
                  <div>
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium">{card.linkedAccount}</span>
                    </div>
                  </div>
                )}
              </div>
